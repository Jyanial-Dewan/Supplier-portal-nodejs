                    ...
                    <div className="px-12 py-8 border shadow-md flex-grow ml-[10rem]">
                        ...
                        {sortedMessages.map((msg) => (
                            <div  key={msg.id} className="grid grid-cols-9 gap-2 py-6 border-b border-gray-200">
                                <Link to={`/notification/sent/${msg.id}`} className="col-start-1 col-end-2">
                                    <p>{msg.recipients.join(', ')}</p>
                                </Link>
                                <Link to={`/notification/sent/${msg.id}`} className="col-start-3 col-end-5">
                                    <p>{msg.subject.slice(0,60)} {msg.subject.length > 60 ? (<span>......</span>) : null}</p>
                                </Link>
                                <Link to={`/notification/sent/${msg.id}`} className="col-start-5 col-end-8">
                                    <p>{msg.body.slice(0,60)} {msg.body.length > 60 ? (<span>......</span>) : null}</p>
                                </Link>
                                <Link to={`/notification/sent/${msg.id}`} className="col-start-8 col-end-8">
                                    <p>{msg.date}</p>
                                </Link>
